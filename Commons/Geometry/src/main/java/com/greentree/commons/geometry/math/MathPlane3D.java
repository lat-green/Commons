...
public class MathPlane3D {
	...
	public MathPlane3D(AbstractVector3f point, AbstractVector3f normal) {
		...
		b = normal.y();
		c = normal.z();
		if(normal.lengthSquared() < Mathf.EPS)throw new IllegalArgumentException(String.format("is not vector (%f, %f, %f)", a, b, c));
	...
	public MathPlane3D(AbstractVector3f p0, AbstractVector3f p1, AbstractVector3f p2) {
		...
		b = x1*z2 - x2*z1;
		c = x1*y2 - x2*y1;
		if(a*a + b*b + c*c < Mathf.EPS)throw new IllegalArgumentException(String.format("is not vector (%f, %f, %f)", a, b, c));
