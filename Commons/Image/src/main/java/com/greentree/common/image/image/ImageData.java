...
import com.greentree.common.image.Color;
...
public final class ImageData implements Serializable {
	...
	public ImageData(byte[] bytes, PixelFormat format, int width, int height) {
		if(width <= 0)  throw new IllegalArgumentException(String.format("width mast by positive %d", width));
		if(height <= 0) throw new IllegalArgumentException(String.format("width mast by positive %d", height));
		if(width * height * format.numComponents != bytes.length) throw new IllegalArgumentException(String.format("size:%dx%d  data.length:%d", width, height, bytes.length));
	...
	public boolean equals(Object obj) {
		...
		return Arrays.equals(data, other.data) && format == other.format && height == other.height && width == other.width;
	}
	...
	public ImageData subImage(int x, int y, int w, int h) {
		...
		for(int j = 0; j < h; j++) for(int i = 0; i < w; i++) for(int c = 0; c < numc; c++) {
	...
	public String toString() {
		return "ImageData [" + Arrays.toString(data) + ", format=" + format + ", width=" + width + ", height=" + height + "]";
	}
