...
public abstract class TypeInfoBuilder {
	...
	public static <C> TypeInfo<C> getTypeInfo(Type type) {
		...
		if(type instanceof WildcardType) {
			return getTypeInfo((WildcardType) type);
		}
	...
	public static <C> TypeInfo<C> getTypeInfo(WildcardType type) {
		if(type.getLowerBounds().length == 0 && type.getUpperBounds().length == 1)
			return getTypeInfo(type.getUpperBounds()[0]);
		return null;
		//		throw new UnsupportedOperationException(
		//				type + " " + Arrays.toString(type.getLowerBounds()) + " " + Arrays.toString(type.getUpperBounds()));
	}
