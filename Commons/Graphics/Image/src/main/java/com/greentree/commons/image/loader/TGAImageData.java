...
import static com.greentree.commons.image.PixelFormat.RGBA;

@Deprecated
public class TGAImageData implements ImageLoader {
    ...
    @Override
    public ByteArrayImageData loadImage(final InputStream fis) throws IOException {
        ...
        if (pixelDepth == 24) {
            if (flipped) for (var i = height - 1; i >= 0; --i)
        ...
        } else if (pixelDepth == 32) if (flipped) for (var i = height - 1; i >= 0; --i)
            for (var j = 0; j < width; ++j) {
                blue = dis.readByte();
                green = dis.readByte();
                red = dis.readByte();
        ...
        if (width < texWidth - 1) for (var y = 0; y < texHeight; ++y)
        ...
        return new ByteArrayImageData(scratch, fmt, texWidth, texHeight);
    }
