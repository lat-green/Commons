...
class ForwardRenderPipeline(private val scene: RenderScene, private val context: RenderContext) : RenderPipeline {
			...
			for(camera in cameras) {
				...
				for(materialGroups in materials) {
					...
					for(meshGroups in materialGroups) {
						val mesh = meshGroups.value
						buffer.bindMesh(mesh)
						val vbo = if(mesh.has(4))
							mesh.vbo(4) as FloatVideoBuffer
						else
							mesh.run {
								val vbo = context.newVideoBuffer().buildFloat()
								add(4, vbo, 4, 4 * 4 * 4, 0 * 4 * 4, 1)
								add(5, vbo, 4, 4 * 4 * 4, 1 * 4 * 4, 1)
								add(6, vbo, 4, 4 * 4 * 4, 2 * 4 * 4, 1)
								add(7, vbo, 4, 4 * 4 * 4, 3 * 4 * 4, 1)
								vbo
							}
						context.newStackBuffer(meshGroups.size * 4 * 4 * 4).use {
							val buf = it.mallocFloat(meshGroups.size * 4 * 4)
...
private fun Mesh.location(location: Int, function: (Mesh) -> VideoBuffer): VideoBuffer {
	if(has(location))
		return vbo(location)
