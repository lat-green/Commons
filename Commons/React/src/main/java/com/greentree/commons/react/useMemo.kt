...
@OptIn(ExperimentalContracts::class)
inline fun <R> ReactContext.useMemoBySame(dependency: Any, block: () -> R): R {
	contract {
		callsInPlace(block, InvocationKind.AT_MOST_ONCE)
	}
	var valueRef by useRef<R>() as Ref<R>
...
@Deprecated("not use Unit as dependency", ReplaceWith("useMemo(block)"))
inline fun <R> ReactContext.useMemoBySame(dependency: Unit, block: () -> R) = useMemo(block)
...
@OptIn(ExperimentalContracts::class)
inline fun <R : AutoCloseable?> ReactContext.useMemoCloseBySame(dependency: Any?, block: () -> R): R {
	contract {
		callsInPlace(block, InvocationKind.AT_MOST_ONCE)
	}
	var valueRef: R by useRefClose<R> {
...
@Deprecated("not use Unit as dependency", ReplaceWith("useMemoClose(block)"))
inline fun <R : AutoCloseable?> ReactContext.useMemoCloseBySame(dependency: Unit, block: () -> R) = useMemoClose(block)