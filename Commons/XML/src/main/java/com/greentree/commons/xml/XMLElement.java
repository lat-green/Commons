...
public class XMLElement implements Serializable {
	...
	public XMLElement(Collection<XMLElement> childrens, Map<String, String> attributes, String name,
			String content) {
	...
	public Collection<XMLElement> getChildrens(final String name) {
		return getChildrens().stream().filter(xml->xml.getName().equals(name))
	...
	public String toString() {
		...
		for(final String n : getAttributeNames())
		...
		if(getChildrens().isEmpty()) {
			...
			if(getContent().indexOf('\n') != -1)
				...
				res.append(getContent());
			for(final XMLElement e : getChildrens())
				res.append(("\n" + e.toString()).replace("\n", "\n\t"));
			res.append("\n</").append(name).append(">");
		}
		return res.toString();
